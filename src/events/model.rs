...
fn model_packet_dispatcher(packet: Packet) {
    ...
    if opt& protocol::IPMSG_SENDCHECKOPT != 0 {
        ...
        let _ = GLOBLE_SENDER.send(ModelEvent::RecMsgReply{packet: recvmsg, from_ip: (&packet).ip.to_owned()});
    }
    if cmd == protocol::IPMSG_BR_EXIT {//收到下线通知消息
        let _ = GLOBLE_SENDER.send(ModelEvent::BroadcastExit((&packet).sender_host.to_owned()));
        ...
        info!("{user:?}");
        let _ = GLOBLE_SENDER.send(ModelEvent::RecOnlineMsgReply{packet: ansentry_packet, from_user: user});
        ...
        let _ = GLOBLE_SENDER.send(ModelEvent::NotifyOnline{user});
        ...
        let _msg_str = if ext_vec.len() > 0 { ext_vec[0] } else { "" };
        //文字消息内容|文件扩展
            ...
            if ext_vec.len() > 1 {
                ...
                for file_str in files {
                    ...
                    if file_attr.len() >= 5 {
                        ...
                        let _ntime = NaiveDateTime::from_timestamp_opt(mmtime_num, 0).unwrap();
